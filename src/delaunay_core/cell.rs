    ...
    V: Clone + Copy + Eq + Hash + Ord + PartialEq + PartialOrd,
{
    ...
    /// let cell: Cell<Option<()>, Option<()>, 3> = CellBuilder::default().vertices(vec![vertex1, vertex2, vertex3]).build().unwrap();
    ...
    /// use uuid::Uuid;
    /// let vertex1 = VertexBuilder::default().point(Point::new([0.0, 0.0, 1.0])).build().unwrap();
    /// let cell: Cell<Option<()>, Option<()>, 3> = CellBuilder::default().vertices(vec![vertex1]).build().unwrap();
    ...
    ///     .build()
    ///     .unwrap();
    /// let cell: Cell<usize, &str, 3> = CellBuilder::default()
    ...
    /// ```
    pub fn contains_vertex_of(&self, cell: &Cell<U, V, D>) -> bool {
    ...
    /// use d_delaunay::delaunay_core::vertex::{Vertex, VertexBuilder};
    /// use d_delaunay::geometry::point::PointND;
    /// let vertex1 = VertexBuilder::default().point(PointND::new([0.0, 0.0, 1.0])).build().unwrap();
    /// let vertex2 = VertexBuilder::default().point(PointND::new([0.0, 1.0, 0.0])).build().unwrap();
    /// let vertex3 = VertexBuilder::default().point(PointND::new([1.0, 0.0, 0.0])).build().unwrap();
    /// let vertex4 = VertexBuilder::default().point(PointND::new([1.0, 1.0, 1.0])).build().unwrap();
    ...
    /// let vertex5 = VertexBuilder::default().point(PointND::new([0.0, 0.0, 0.0])).build().unwrap();
    ...
    /// let vertex2 = VertexBuilder::default().point(PointND::new([0.0, 1.0, 0.0])).build().unwrap();
    /// let vertex3 = VertexBuilder::default().point(PointND::new([1.0, 0.0, 0.0])).build().unwrap();
    /// let vertex4 = VertexBuilder::default().point(PointND::new([0.0, 0.0, 0.0])).build().unwrap();
    /// let cell: Cell<usize, Option<()>, 3> = CellBuilder::default().vertices(vec![vertex1, vertex2, vertex3, vertex4]).build().unwrap();
    ...
    V: Clone + Copy + Eq + Hash + Ord + PartialEq + PartialOrd,
{
    ...
    /// ```
    pub fn circumcenter(&self) -> Result<PointND<D>, anyhow::Error> {
        let dim = self.dim();
        ...
        let coords_0: [f64; D] = (&self.vertices[0]).into();

        for i in 0..dim {
            let coords_i: [f64; D] = (&self.vertices[i + 1]).into();

            // Fill matrix row
            for j in 0..dim {
                matrix[(i, j)] = coords_i[j] - coords_0[j];
            }

            // Fill vector element
            b[(i, 0)] =
                na::distance_squared(&na::Point::from(coords_i), &na::Point::from(coords_0));
        }
        ...
        Ok(PointND::<D>::new(solution_array))
    }
    ...
    pub fn circumradius(&self) -> Result<f64, anyhow::Error> {
        let circumcenter = self.circumcenter()?;
        Ok(self.circumradius_with_center(&circumcenter))
    }

    /// Alternative method that accepts precomputed circumcenter
    fn circumradius_with_center(&self, circumcenter: &PointND<D>) -> f64 {
        let vertex_coords: [f64; D] = (&self.vertices[0]).into();
        na::distance(
            &na::Point::from(circumcenter.coordinates()),
            &na::Point::from(vertex_coords),
        )
    }
    ...
    pub fn circumsphere_contains(&self, vertex: VertexND<U, D>) -> Result<bool, anyhow::Error> {
        ...
        let vertex_coords: [f64; D] = (&vertex).into();
    ...
    pub fn circumsphere_contains_vertex(
        ...
        for (i, v) in self.vertices.iter().enumerate() {
            // Use implicit conversion from vertex to coordinates
            let vertex_coords: [f64; D] = v.into();
            for j in 0..D {
                matrix[(i, j)] = vertex_coords[j];
            }
        ...
        // Use implicit conversion from vertex to coordinates
        let test_vertex_coords: [f64; D] = (&vertex).into();
        for j in 0..D {
            matrix[(D, j)] = test_vertex_coords[j];
        }
        matrix[(D, D)] = 1.0;
    ...
    /// ```
    pub fn facets(&self) -> Vec<Facet<U, V, D>> {
        self.vertices
            .iter()
    ...
    V: Clone + Copy + Eq + Hash + Ord + PartialEq + PartialOrd,
{
    ...
    fn eq(&self, other: &Self) -> bool {
        sorted_vertices::<U, D>(&self.vertices) == sorted_vertices::<U, D>(&other.vertices)
    }
}
    ...
    V: Clone + Copy + Eq + Hash + Ord + PartialEq + PartialOrd,
{
    ...
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        sorted_vertices::<U, D>(&self.vertices)
            .partial_cmp(&sorted_vertices::<U, D>(&other.vertices))
    }
}
    ...
    V: Clone + Copy + Eq + Hash + Ord + PartialEq + PartialOrd,
{
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        ...
        for vertex in &sorted_vertices::<U, D>(&self.vertices) {
            vertex.hash(state);
        }
