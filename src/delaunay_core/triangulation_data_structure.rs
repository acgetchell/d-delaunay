...
fn facets_are_adjacent<U, V, const D: usize>(
    facet1: &Facet<U, V, D>,
    facet2: &Facet<U, V, D>,
) -> bool
where
    ...
    [T; D]: Copy + Default + Serialize + Sized,
{
    ...
    pub cells: HashMap<Uuid, Cell<U, V, D>>,
}
    ...
    ordered_float::OrderedFloat<f64>: From<T>,
{
    ...
    pub fn add(&mut self, vertex: VertexND<U, D>) -> Result<(), &'static str> {
        ...
        for val in self.vertices.values() {
            ...
            let new_coords: [f64; D] = (&vertex).into();
            if existing_coords
                .iter()
                .zip(new_coords.iter())
    ...
    fn supercell(&self) -> Result<Cell<U, V, D>, anyhow::Error> {
        ...
        for i in 0..D {
            let min_f64: f64 = min_coords[i];
            let max_f64: f64 = max_coords[i];
    ...
    fn create_default_supercell() -> Cell<U, V, D> {
    ...
    fn create_supercell_simplex(center: &[T; D], radius: T) -> Vec<PointND<D>> {
        ...
        if D == 3 {
            ...
            for vertex_coords in &tetrahedron_vertices {
                ...
                points.push(PointND::new(coords));
            }
        } else {
            ...
            for i in 0..D {
                let center_f64: f64 = center[i].into();
                coords[i] = center_f64 + radius_f64;
            }
            points.push(PointND::new(coords));
    ...
    fn remove_cells_containing_supercell_vertices(&mut self, _supercell: &Cell<U, V, D>) {
