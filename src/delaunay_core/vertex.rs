    ...
    U: Clone + Copy + Eq + Hash + Ord + PartialEq + PartialOrd,
{
    ...
    #[inline]
    #[must_use]
    pub fn from_points(points: Vec<PointND<D>>) -> Vec<Self> {
        points
            .into_iter()
    ...
    /// let another_vertex: Vertex<Option<()>, 3> = VertexBuilder::default().point(point).build().unwrap();
    ...
    ///
    /// let invalid_vertex: Vertex<Option<()>, 3> = VertexBuilder::default()
...
where
    U: Clone + Copy + Eq + Hash + Ord + PartialEq + PartialOrd,
{
    /// Generic Hash implementation for Vertex
